    ...
    <section id="works" className="section px-4 sm:px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 gap-8 mb-12">
          {works.map((work) => (
            <div key={work.title} className="relative h-48 sm:h-64 overflow-hidden rounded-2xl group">
              <img 
                src={work.image} 
                alt={work.title}
        ...
        <div className="grid sm:grid-cols-2 gap-8">
          {works.map((work) => (
            ...
            >
              <h3 className="text-xl sm:text-2xl font-bold mb-4 group-hover:text-blue-500 transition-colors">{work.title}</h3>
